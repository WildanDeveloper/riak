    ...
    fn dfs(&mut self, w: &[Word; 4], r: usize, log2dp: f64) {
        ...
        if self.nodes >= self.last_report {
            ...
            eprintln!(
                ...
                self.nodes, el, self.nodes as f64 / el,
                if self.best_trail.is_empty() { "none".into() } else { format!("2^{:.1}", self.best_log2dp) }
            );
            self.last_report = self.nodes + 2_000_000;
        }
        if r == ROUNDS {
            ...
            if !self.trail.is_empty() && log2dp > self.best_log2dp {
                ...
                eprintln!("[hit] nodes={} best=2^{:.1} trail_len={}", self.nodes, log2dp, self.trail.len());
            }
            return;
        }
